...
```{r}
K = seq(1:20)
MSE.frame = numeric(length(K))
for (i in 1:length(K)){
  predictions = knn.reg( , , , k=K[i])
  MSE.frame[i] = mean((Auto$mpg[-train] - predictions$pred)^2)
}
plot(K,MSE.frame,type="l")
```
...
I believe that K is the best selection for K because it produces the lowest MSE value. 
